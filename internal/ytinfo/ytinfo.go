...
type Meta struct {
	ID     string // canonical video ID, stable across URL forms
...
func Lookup(url string) Meta {
	...
	// One yt-dlp call prints the ID, title, and channel, each on its own line.
	cmd := exec.CommandContext(ctx, "yt-dlp", "--no-playlist", "--skip-download",
		"--print", "id", "--print", "title", "--print", "channel", url)
	out, err := cmd.Output()
	if err != nil {
		return Meta{}
	}
	...
	var m Meta
	if len(lines) > 0 {
		if id := strings.TrimSpace(lines[0]); id != "NA" {
			m.ID = id
		}
	}
	if len(lines) > 1 {
		m.Title = strings.TrimSpace(lines[1])
	}
	if len(lines) > 2 {
		if artist := strings.TrimSpace(lines[2]); artist != "NA" {
			m.Artist = artist
		}
	}
	return m
